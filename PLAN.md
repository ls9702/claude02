...
| RAM 1GB | 프로덕션 실측(x86): app 108~140MB(일반 부하), 스트레스 시 372MB / room 80~99MB → compose `mem_limit` app 384m·room 144m, 총 528MB. 스냅샷 보관 20개, 이미지 리사이즈 2048px. NAS 실측 후 재조정 (SETUP.md 9절) |
...
| Fortune-sheet 엔진 제약 (1.0.4 정식 릴리스로 pre-1.0 리스크는 해소) | 실측된 제약: 시트 간 참조는 ASCII 시트명만, 배열 수식(SUMPRODUCT) 미지원, TEXT() 미지원, 수식 조건부 서식 크래시 → 월별 요약을 같은 시트 블록으로, 숫자 보조 열 + SUMIF, 값 기반 서식으로 대체. 저장 포맷은 자체 스키마로 감싸 엔진 교체 여지 유지 (KNOWN_ISSUES §15) |
