/**
 * WebCrypto AES-GCM 헬퍼.
 *
 * 출처: `@excalidraw/excalidraw` 패키지의 `data/encryption.ts` (MIT).
 * npm 배포본은 이 모듈의 **타입만** 노출하고 JS 는 내보내지 않으므로 원본을 복사했다.
 * (`blobToArrayBuffer` 의존만 인라인으로 풀었다.)
 */
...
export const getCryptoKey = (key: string, usage: KeyUsage): Promise<CryptoKey> =>
